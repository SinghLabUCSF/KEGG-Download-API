...
# 20170316 updated to adjust gene name parsing
# 20170405 updated again to fix a bug for gene name parsing
...
foreach (<GENES>){
	...
	# ***** IMPORTANT: Check the pattern to parse genes. It changes from version to version.	
	$line[1] =~s/.+\(RefSeq\)//g;
	
	if ($line[1] =~/,|;/){
		my @names = split /[\,\;]/, $line[1];
		#print "$line[0]\t$names[0]\n";
...
foreach (<PATH>){
	...
	if ($organism eq 'cel'){$line[1] =~s/ - Caenorhabditis elegans \(nematode\)//g}
...
# The input style can be changed here for a single line or multi line output
